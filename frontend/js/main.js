...
// ✅ MODIFIKASI: Menambahkan totalRssi dan avgRssi
let realtimeStats = {
...
async function handleAuthMessage(data) {
    try {
        ...
        // ✅ AMBIL METADATA BARU (AuthDelay dan RSSI)
        ...
        if (device === currentDevice) {
            ...
            // ✅ Setelah Auth, refresh tabel log via DeviceManager
...
async function handleParamMessage(data) {
    try {
        ...
        // ✅ REAL: Waktu ESP32 mengirim (sentTime dari Arduino/ESP-IDF atau publishTime_us dari ESP-IDF)
        ...
        // ✅ Ambil RSSI dari payload param (jika ada, dari ESP32-CAM)
...
            messageSize: msgSize,
            qos: data.qos || 1,
            // ✅ Mengirim waktu terima browser (ms) agar backend bisa hitung delay server
            sentTime: browserReceiveTime, 
...
    const icon = data.status === 'success' ? '✅' : '❌';
    ...
    // ✅ Logika tambahan agar log activity di-refresh oleh DeviceManager (yang memuat data lengkap dari backend)
    if (deviceManager) {
